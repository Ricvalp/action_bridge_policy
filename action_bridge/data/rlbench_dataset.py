"""Standard state-conditioned imitation-learning windows from RLBench caches."""
...
def encode_action_chunk(
    ...
    *,
    ...
    """Encode absolute RLBench gripper targets for model supervision."""
...
def decode_action_chunk(
    ...
    *,
    ...
    """Convert a model chunk back to absolute RLBench gripper targets."""
    ...
    if representation == "absolute":
        return actions
    output = actions.copy()
    anchor = np.asarray(observation_state, dtype=np.float32)[-1:, :3]
    output[:, :3] = np.cumsum(actions[:, :3], axis=0) + anchor
    return output
...
def encode_action_history(
    ...
    *,
    ...
    """Encode past executed targets in the same coordinates as future actions."""
...
def split_episode_ids(
    ...
    *,
    ...
    """Deterministically split episodes within each task variation."""
...
class RLBenchDataset(Dataset):
    """PyTorch dataset for standard, state-conditioned action generation.

    The cache remains episode-shaped. Observation history, action history,
    future horizon, stride, coordinate representation, point count, and split
    are all selected here and can change without rebuilding the cache.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if len(set(dimensions)) != 1:
            details = {
                f"{key.task}:variation{key.variation}": dims
                for key, dims in zip(self.keys, dimensions)
            }
            raise ValueError(
                "Selected RLBench caches have inconsistent "
                f"(point_count, state_dim, action_dim): {details}"
            )
        ...
        if self.point_count > self.cached_point_count:
            raise ValueError(
                f"point_count={self.point_count} exceeds cached point count "
                f"{self.cached_point_count}."
            )
        ...
        for variation_index, key in enumerate(self.keys):
            ...
            available = set(
                self.store.available_fields(variation_index, int(episode_ids[0]))
            )
            missing = sorted(required_fields - available)
            if missing:
                raise ValueError(
                    f"{key.task}/variation{key.variation} is missing requested cache "
                    f"fields {missing}. Rebuild with those fields or disable them in "
                    "RLBenchDataset."
                )
        ...
        for variation_index, key in enumerate(self.keys):
            episode_ids = split_episode_ids(
                ...
                seed=int(split_seed)
                + zlib.crc32(f"{key.task}:{key.variation}".encode("utf-8")),
            )
        ...
        if not self.indices:
            raise ValueError(
                f"No valid RLBench windows for split={self.split!r}. Check episode lengths, "
                "split fractions, history lengths, action_offset, and chunk_horizon."
            )
    ...
    def set_epoch(self, epoch: int) -> None:
        """Change deterministic random point subsets between training epochs."""

        self._epoch = int(epoch)

    def sampling_weights(self, strategy: str = "window_uniform") -> torch.Tensor:
        """Return weights for a PyTorch ``WeightedRandomSampler``.

        ``window_uniform`` reproduces ordinary shuffled loading. Other choices
        prevent long episodes, variations with more frames, or tasks with more
        variations from dominating solely because they create more windows.
        """

        strategy = str(strategy)
        if strategy == "window_uniform":
            return torch.ones(len(self.indices), dtype=torch.double)
        if strategy == "episode_uniform":
            groups = [
                (window.variation_index, window.episode_id)
                for window in self.indices
            ]
        elif strategy == "variation_uniform":
            groups = [window.variation_index for window in self.indices]
        elif strategy == "task_uniform":
            groups = [
                self.keys[window.variation_index].task
                for window in self.indices
            ]
        else:
            raise ValueError(
        ...
        return torch.as_tensor(
            [1.0 / counts[group] for group in groups],
            dtype=torch.double,
        )
    ...
    def _window_indices(
        ...
        future_raw = future_start + np.arange(self.chunk_horizon, dtype=np.int64) * self.action_stride
    ...
    def _point_indices(self, dataset_index: int, valid: np.ndarray) -> np.ndarray:
        ...
        return np.asarray(
            rng.choice(usable, size=count, replace=usable.size < count),
            dtype=np.int64,
        )
    ...
    def item_from_episode_time(
        ...
        *,
        ...
        observation_fields = list(dict.fromkeys(observation_fields))
        ...
        previous_hist_idx = np.clip(
            hist_idx - self.action_stride,
            0,
            length - 1,
        )
        ...
        xyz = observations["xyz"][:, point_indices].astype(np.float32)
        point_valid = observations["valid"][:, point_indices].astype(np.bool_)
        key = self.keys[int(variation_index)]
        task_variation_name = f"{key.task}:{key.variation}"

        output: Dict[str, Any] = {
            "obs_hist": self._to_tensor(state),
            "point_cloud_hist": self._to_tensor(xyz),
            "point_valid_hist": self._to_tensor(point_valid),
            "act_hist": self._to_tensor(history),
            "future_actions": self._to_tensor(future),
            "obs_history_mask": self._to_tensor(obs_mask),
            "action_history_mask": self._to_tensor(hist_mask),
            "future_action_mask": self._to_tensor(future_mask),
            "action_is_absolute": torch.tensor(
                self.action_representation == "absolute",
                dtype=torch.bool,
            ),
            "context": {
                "task_id": torch.tensor(self.task_to_id[key.task], dtype=torch.long),
                "task_variation_id": torch.tensor(
                    self.task_variation_to_id[task_variation_name],
                    dtype=torch.long,
                ),
                "variation_id": torch.tensor(key.variation, dtype=torch.long),
        ...
        if self.return_absolute_actions and self.action_representation != "absolute":
            output["future_actions_absolute"] = self._to_tensor(future_absolute)
        if self.include_rgb:
            output["rgb_hist"] = self._to_tensor(
                observations["rgb"][:, point_indices].astype(np.float32) / 255.0
            )
        if self.include_mask_id:
            output["mask_id_hist"] = self._to_tensor(
                observations["mask_id"][:, point_indices]
            )
        if self.extra_observation_fields:
            output["low_dim"] = {
                field: self._to_tensor(np.asarray(observations[field]))
