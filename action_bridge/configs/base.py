...
def _logging_config() -> ConfigDict:
    ...
    config.sim_eval_n_exec = None
    config.sim_eval_seed = None
    config.sim_eval_render_episodes = 0
    config.sim_eval_plot_episodes = 8
    config.sim_eval_save_gifs = False
...
def pusht_lowdim_config(latent_type: str) -> ConfigDict:
    ...
    config.eval.wrong_side_num_samples = 32
    config.eval.wrong_side_executed_k = 3
    config.eval.wrong_side_offsets = [140.0, 100.0, 60.0, 20.0]
    config.eval.wrong_side_clearance = 38.0
    config.eval.wrong_side_t_long_extent = 120.0
...
def isaaclab_franka_cube_lift_config(latent_type: str) -> ConfigDict:
    """Return the first native-Isaac-Lab imitation-learning config.

    This is a low-dimensional, absolute end-effector-pose baseline for the
    Franka cube-lift task.  Isaac Lab remains an online-runtime dependency;
    offline training only consumes a validated ``phi-isaaclab`` collection.
    The defaults establish an integration baseline and are not tuned results.
    """

    config = _toy_common("isaaclab_franka_cube_lift")
    config.device = "cuda"
    config.chunk_horizon = 4
    config.obs_history = 2
    config.action_history = 2
    config.obs_dim = 35
    config.action_dim = 8
    config.output_dir = "workspace/experiments/isaaclab"
    ...
    config.eval.batch_size = 256
    config.eval.offline_max_batches = 0
    config.eval.online_episodes = 100
    config.eval.online_seed = 1_000_000
    config.eval.online_max_steps = 300
    config.eval.online_num_envs = 64
    config.eval.actions_per_plan = 1
    config.eval.record_video = False
    return config
