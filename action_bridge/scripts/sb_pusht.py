...
def audit(root, dataset=None):
    ...
    paths = subprocess.run(["rg", "-l", "--hidden", "--no-ignore", "-g", "*.py",
                            "-g", "!**/.git/**", "-g", "!**/site-packages/**",
                            "-g", "!**/.venv*/**", "-g", "!**/.native/**",
                            "-g", "!**/.uv-cache*/**", "-g", "!**/node_modules/**",
                            "DDIMScheduler|DDPMScheduler|flow_matching|class .*EMA",
    ...
    report = dict(commit=git("rev-parse", "HEAD"), worktree=git("status", "--short"),
                  ...
                  discovered_implementation_paths=paths.stdout.splitlines(),
...
def run_stage(stage, root, dataset, config, device):
    ...
    if stage == "prepare":
        ...
        if manifest_path.exists():
            ...
            if manifest["config"] != config or manifest["metadata"] != metadata:
    ...
    if stage == "reference":
        return fit_completion(windows["train"], windows["val"], config,
                              root / "reference", metadata, device)
    if stage == "ddim":
        from action_bridge.eval.revision_pusht import evaluate
        def validate(policy, step):
            return evaluate(policy, config, metadata, {}, device, seeds=config["validation_seeds"])
        return train(windows["train"], config, root / "ddim", metadata, {}, device, validate=validate)
    ...
    from action_bridge.eval.revision_pusht import evaluate
    def validate(policy, step):
        return evaluate(policy, config, metadata, dependencies, device, seeds=config["validation_seeds"])
    ...
    return train(sources["records"]["train"], config, root / stage, metadata,
                 dependencies, device, pairer=pairer, validate=validate)
...
def main(argv=None):
    ...
    for stage in stages:
        ...
        run_stage(stage, output, args.dataset, config, args.device)
